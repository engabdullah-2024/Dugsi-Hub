...
const SourcesPage = () => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-white via-pink-50 to-pink-200 py-16 px-4 sm:px-8">
        ...
        <div className="flex flex-wrap justify-center gap-4 mb-12">
          {subjects.map(({ key, icon }) => (
            <button
              ...
              className={`flex items-center gap-2 px-6 py-2 rounded-full text-base font-semibold transition-all duration-300 shadow-sm ${
                selectedSubject === key
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {videos[selectedSubject]?.length > 0 ? (
            videos[selectedSubject].map(({ title, url }, idx) => (
              <div
                key={idx}
                className="bg-white rounded-2xl shadow-lg hover:shadow-pink-200 overflow-hidden transition transform hover:-translate-y-1 duration-300"
              >
